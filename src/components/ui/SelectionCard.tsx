  ...
  return (
    <div
      ...
      className={`group relative p-6 rounded-2xl border transition-all duration-300 cursor-pointer select-none outline-none ${
        isSelected
          ? 'bg-gradient-to-b from-cyan-500/10 via-bg-secondary to-bg-secondary border-cyan-400/60 shadow-lg shadow-cyan-500/15 ring-2 ring-cyan-500/30'
    ...
    >
          ...
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 ${
              isSelected
                ...
                : 'bg-bg-tertiary text-text-secondary group-hover:text-cyan-400 group-hover:bg-cyan-500/10'
            }`}
          >
      ...
      <div className="mt-5 space-y-2">
        <h3 className="text-lg font-semibold text-text-primary group-hover:text-cyan-400 transition-colors flex items-center justify-between">
          <span>{option.title}</span>
          <ChevronRight className={`w-4 h-4 text-text-muted transition-transform duration-200 ${isSelected ? 'translate-x-1 text-cyan-400' : 'group-hover:translate-x-1'}`} />
        </h3>
